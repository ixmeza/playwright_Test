...
class Frame {
    ...
    async waitForSelector(selector, options) {
        ...
        if (options && options.waitFor && options.waitFor !== 'visible')
            throw new Error('options.waitFor is not supported, did you mean options.state?');
        const { state = 'visible' } = (options || {});
        if (!['attached', 'detached', 'visible', 'hidden'].includes(state))
            throw new Error(`Unsupported waitFor option "${state}"`);
        const deadline = this._page._timeoutSettings.computeDeadline(options);
        const { world, task } = selectors_1.selectors._waitForSelectorTask(selector, state, deadline);
    ...
    async _retryWithSelectorIfNotConnected(actionName, selector, options, action) {
        const deadline = this._page._timeoutSettings.computeDeadline(options);
        ...
        throw new errors_1.TimeoutError(`waiting for selector "${selector}" failed: timeout exceeded. Re-run with the DEBUG=pw:input env variable to see the debug log.`);
    }
    async click(selector, options = {}) {
        await this._retryWithSelectorIfNotConnected('click', selector, options, (handle, deadline) => handle.click(helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async dblclick(selector, options = {}) {
        await this._retryWithSelectorIfNotConnected('dblclick', selector, options, (handle, deadline) => handle.dblclick(helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async fill(selector, value, options = {}) {
        await this._retryWithSelectorIfNotConnected('fill', selector, options, (handle, deadline) => handle.fill(value, helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async focus(selector, options = {}) {
        await this._retryWithSelectorIfNotConnected('focus', selector, options, (handle, deadline) => handle.focus());
    }
    async hover(selector, options = {}) {
        await this._retryWithSelectorIfNotConnected('hover', selector, options, (handle, deadline) => handle.hover(helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async selectOption(selector, values, options = {}) {
        return await this._retryWithSelectorIfNotConnected('selectOption', selector, options, (handle, deadline) => handle.selectOption(values, helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async setInputFiles(selector, files, options = {}) {
        await this._retryWithSelectorIfNotConnected('setInputFiles', selector, options, (handle, deadline) => handle.setInputFiles(files, helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async type(selector, text, options = {}) {
        await this._retryWithSelectorIfNotConnected('type', selector, options, (handle, deadline) => handle.type(text, helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async press(selector, key, options = {}) {
        await this._retryWithSelectorIfNotConnected('press', selector, options, (handle, deadline) => handle.press(key, helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async check(selector, options = {}) {
        await this._retryWithSelectorIfNotConnected('check', selector, options, (handle, deadline) => handle.check(helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
    async uncheck(selector, options = {}) {
        await this._retryWithSelectorIfNotConnected('uncheck', selector, options, (handle, deadline) => handle.uncheck(helper_1.helper.optionsWithUpdatedTimeout(options, deadline)));
    }
...
class RerunnableTask {
    constructor(data, task, deadline, title) {
        ...
        // timeout on our end.
        const timeoutError = new errors_1.TimeoutError(`waiting for ${title || 'function'} failed: timeout exceeded. Re-run with the DEBUG=pw:input env variable to see the debug log.`);
...
function selectorToString(selector, state) {
    let label;
    switch (state) {
        case 'visible':
            label = '[visible] ';
